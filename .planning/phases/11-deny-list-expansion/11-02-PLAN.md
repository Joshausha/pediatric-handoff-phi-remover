...
<instructions>
**PREREQUISITE VERIFICATION**: Before proceeding, verify that Plan 11-01 Task 2 completed successfully:
```bash
grep -A5 "LOCATION" app/deidentification.py | grep "any(term.lower() in detected_lower"
```
This should show the substring matching code. If not found, Plan 11-01 must be completed first.
