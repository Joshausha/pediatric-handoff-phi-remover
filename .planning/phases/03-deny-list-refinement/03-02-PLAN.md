...
<objective>
Mirror deny list changes in test files and add regression tests.

Purpose: Ensure evaluation and calibration scripts use the same deny list filtering as production code. Add regression tests to prevent future false positive regressions.
Output: Updated test files with consistent deny list handling and new regression tests.
</objective>
...
- DENY-04: Regression tests verify false positive prevention
