...
<task type="auto">
  ...
  <action>
Create a new test class `TestHyphenatedRoomEdgeCases` with parameterized tests for standalone hyphenated rooms:
...
Use pytest parameterization decorator:
```python
@pytest.mark.parametrize("room,text", HYPHENATED_ROOM_EDGE_CASES)
def test_hyphenated_room_detected(room, text):
    result = deidentify_text(text)
    assert "[ROOM]" in result, f"Expected [ROOM] in: {result}"
    assert room not in result, f"Room {room} should be redacted"

@pytest.mark.parametrize("number,text", HYPHENATED_NON_ROOM_CASES)
def test_hyphenated_non_room_not_overdetected(number, text):
    result = deidentify_text(text)
    assert number in result, f"Non-room {number} should NOT be redacted"
```

Each positive test should:
...
1. Call `deidentify_text()` with the context sentence
2. Assert the original number STILL appears (not over-redacted)
  </action>
  <verify>
Run `pytest tests/test_deidentification.py::TestHyphenatedRoomEdgeCases -v` and verify room tests pass while non-room tests correctly don't over-detect.
  </verify>
  <done>8 hyphenated room tests pass, and 2 negative tests confirm no over-detection on age/date ranges.</done>
</task>
...
<verification>
1. `pytest tests/test_deidentification.py::TestHyphenatedRoomEdgeCases -v` - 8 positive tests pass, 2 negative tests pass
