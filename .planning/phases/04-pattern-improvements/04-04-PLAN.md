...
<task type="auto">
  <name>Task 3: Run validation to measure improvement</name>
  <files>tests/results/phone_improvement.json</files>
  <action>
Run the evaluation script on the standard dataset to measure recall improvement for PHONE_NUMBER:

```bash
python scripts/evaluate_phi.py tests/synthetic_handoffs.json --output tests/results/phone_improvement.json
```

Extract the PHONE_NUMBER recall from the results. Expected improvement from 73.9% to >90%.

If the script doesn't exist or errors, run a quick manual test:
```bash
python -c "
from app.deidentification import deidentify_text
tests = [
    '001-411-671-8227',
    '+1-899-904-9027x87429',
    '538.372.6247',
    '(392)832-2602x56342',
]
for t in tests:
    result = deidentify_text(f'Contact at {t}')
    print(f'{t}: {\"PASS\" if t not in result else \"FAIL\"}')"
```
  </action>
  <verify>
All 4 quick test patterns show "PASS" indicating phone numbers are being detected.
  </verify>
  <done>PHONE_NUMBER recall improved and verified with quick tests.</done>
</task>
