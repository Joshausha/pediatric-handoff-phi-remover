...
must_haves:
  truths:
    ...
    - "All 8 entity types have documented recall/precision/F1/F2 scores"
...
Output: Comprehensive BASELINE_METRICS.md with timestamped metrics, per-entity performance, known limitations, and test failure analysis.
</objective>
...
Record all captured data for use in Task 2.
  ...
  <verify>
Files exist: `/tmp/baseline_report.txt`, `/tmp/baseline_metrics.json`, `/tmp/baseline_confusion.json`, `/tmp/test_results.txt`
  </verify>
...
3. **No speech artifacts**: Dataset doesn't include stutters, corrections, "um", "uh"
4. **Limited name diversity**: Faker defaults may not represent diverse patient populations
5. **No adversarial cases**: Edge cases like punctuation variations, start-of-line positions not tested
6. **Threshold not calibrated**: Detection threshold (0.35) set arbitrarily, not data-driven
...
3. All 8 entity types are documented with TP/FN/FP/P/R/F1/F2
4. Overall metrics match JSON output from evaluate_presidio.py
5. Known limitations section has actionable items for future phases
6. Document references correct file paths and commands
</verification>

<success_criteria>
1. BASELINE_METRICS.md created with complete structure
2. Metrics are accurate (match evaluation script output)
3. Per-entity breakdown covers all 8 PHI types
4. Known limitations documented for future phases
