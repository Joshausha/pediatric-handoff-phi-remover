...
must_haves:
  ...
  key_links:
    ...
    - from: "tests/validation_dataset.py"
      ...
      via: "Compatible with PresidioEvaluator"
      pattern: "phi_spans"
---
...
- Property phi_spans that converts phi_annotations to PHISpan objects (for compatibility with PresidioEvaluator)
...
spans = h.phi_spans
print(f'ValidationHandoff created with {len(spans)} spans')
...
- [ ] ValidationHandoff.phi_spans returns PHISpan-compatible objects (with start, end, entity_type, text attributes)
