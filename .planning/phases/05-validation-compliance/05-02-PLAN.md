...
must_haves:
  truths:
    - "Bootstrap confidence intervals calculated for recall/precision metrics"
    - "False negatives categorized by failure mode (pattern_miss, threshold_miss, etc.)"
    - "CI width reported alongside point estimates"
  artifacts:
  ...
  key_links:
    - from: "tests/evaluate_presidio.py"
      ...
      via: "Error taxonomy integration"
