...
must_haves:
  truths:
    - "Clinician can run a single command to validate PHI detection performance"
    - "Clinician can see recall with confidence bounds to assess safety margin"
    - "Clinician can see deployment recommendation based on 95% threshold"
  artifacts:
...
Purpose: Execute validation on available data (synthetic corpus as MVP validation - external validation on real transcripts deferred until IRB-approved prospective collection), calculate recall with 95% CI, generate HIPAA-compliant documentation, and determine if 95% recall threshold is met.

Note on VALD-01 (external validation): For this personal QI project, synthetic data validation serves as the practical validation path. Real external validation requires IRB approval for prospective transcript collection. The synthetic corpus is designed to represent realistic clinical patterns based on the developer's clinical experience. This limitation is documented in the VALIDATION_REPORT.md.
...
1. **Synthetic data validation (MVP)**: Current validation uses synthetic handoffs designed from clinical experience; external validation on real transcripts requires IRB-approved prospective collection (deferred to Phase 6)
2. **Domain shift**: Performance on real clinical transcripts may differ from synthetic (literature suggests 15-30% gap)
3. **Single annotator**: Ground truth from synthetic generation, not human expert annotation (expert review conducted in Plan 05-04)
